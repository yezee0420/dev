...
| 구독 카탈로그·시드 데이터 | [app/data/seed-subscriptions.json](app/data/seed-subscriptions.json) |
...
7. **커밋·배포는 사용자가 터미널 `gitship`으로 수행.** Claude의 `/git-ship` 슬래시 커맨드도 필요시 쓸 수 있지만, **두 개를 동시에 실행하지 않는다** (stash·rebase 충돌 위험). Claude가 Git 조작을 할 때는 기본적으로 `git add`, `git commit`까지만. 상세: `docs/rules/git-workflow.md`.
