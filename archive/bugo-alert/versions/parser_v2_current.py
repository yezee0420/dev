...
def parse_title(title: str) -> ParsedObituary:
    ...
    # 패턴6a: [부고] 이름 씨(소속) 별세 — 씨가 괄호 앞 (윤호상 씨(피학살자유족회...) 별세)
    m = re.search(
        r"[\[<\(]\s*부고\s*[\]>\)]\s*(?P<name>[가-힣]{2,5})\s*씨\s*\((?P<affil>[^)]+)\)\s*별세",
        title,
    )
    if m:
        result.key_person = _take_first_name(_strip_suffix(m.group("name")))
...
# 관계(고인씨·나이)상 — 모친(김용연씨·97)상, 모친(조춘옥씨·95세)상
_RE_DECEASED_REL = re.compile(
    r"(?:모친|부친|장인|장모|시부|시모|조부|조모|외조부|외조모|남편|부인|빙모|빙부)\s*"
    r"\(\s*(?P<name>[가-힣]{2,5})\s*씨\s*[·\s]\s*(?P<age>\d+세?)\s*\)\s*상"
)
...
# 장례식장: '장레식장' '장례예식장' 오타, '장례문화원' '국화원' '백합원' 포함
_RE_FUNERAL_HALL = re.compile(
    r"(?P<hall>[가-힣A-Za-z\s]{2,}(?:병원|의료원|대학교?|센터|시민|농협|공원|빛고을|메모리얼|종합)\s*장[례레]예?식장|"
    r"[가-힣A-Za-z\s]+(?:병원|의료원)\s+(?:국화원|백합원)|"
    r"[가-힣A-Za-z]+(?:장[례레]예?식장|장례문화원))\s*"
    r"(?P<room>(?:특?\d+호실|[가-힣]+\d*호실|VIP\s*\d*호?|특실|\d+호|\(\d+층\)\s*\d+호실))?"
)
...
def parse_body(body: str) -> dict:
    ...
    # 복수 부고 본문: ◇A씨 별세·B 소속 직급 관계상 — A=고인, B=핵심인물
    _RE_COMPOSITE = re.compile(
        r"[◇◆▲△]\s*(?P<deceased>[가-힣]{2,5})\s*씨\s*별세\s*[··]\s*"
        r"(?P<name>[가-힣]{2,5})\s+(?P<rest>[^=]+)\s+(?P<rel>[가-힣]+상)"
    )
    cm = _RE_COMPOSITE.search(body)
    if cm:
        if not result.get("deceased_name"):
            result["deceased_name"] = _strip_suffix(cm.group("deceased"))
        rest = cm.group("rest").strip()
        org, pos = _split_rest_to_org_pos(rest)
        result["key_person"] = _strip_suffix(cm.group("name"))
        result["organization"] = org
        result["position"] = pos
        result["relationship"] = cm.group("rel")
...
def parse_obituary(title: str, body: str) -> ParsedObituary | None:
    ...
    # 복수 부고 본문(◇A씨 별세·B 관계상)에서 핵심인물 추출 시 본문 우선
